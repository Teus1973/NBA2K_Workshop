...
def apply_to_prospect(
    ...
    *,
    ...
    if config.get_rating_engine() == "excel_2026_class":
        from .excel_2026_class import apply_excel_2026_to_prospect

        computed, _prov = apply_excel_2026_to_prospect(prospect, registry)
        for k, v in _prov.items():
            provenance.mark(k, v)
        _apply_combine_override(computed, provenance, prospect)
        _apply_scouting_physical_proxy(computed, provenance, prospect)
        overall_features: dict[str, float] = dict(feats)
        overall_features.update(
            {k: float(v) for k, v in computed.items() if k != "overall_2k"},
        )
    ...
    # 2b. When combine inputs are missing, nudge physicals from AI scouting 0–1 hints.
    _apply_scouting_physical_proxy(computed, provenance, prospect)
    ...
    # 4b. If we have no college/g league stat line, regression sits near intercept.
    #    Gently pull overall toward a plausible band from big-board rank (only
    #    overall; sub-attributes still come from the formula when available).
    _nudge_overall_for_missing_stats(
        computed, provenance, prospect, feats)
